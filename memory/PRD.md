...
- 2026-06-08 (iter 11): Iteración 11 — **Módulo Gastos Fijos / Operación** (`/operacion`). Nuevo backend endpoint `GET /api/operations/recurring?months_back=12` con detección automática: agrupa egresos operativos (sin proyecto + provider_id + paid_personally=false) por proveedor, calcula intervalo promedio entre pagos y clasifica frecuencia (Semanal 6-9d, Quincenal 13-17d, Mensual 25-35d, Cada 45d 40-50d, Bimestral 55-65d, Trimestral 85-95d, Semestral 175-190d, Anual 355-375d, o Irregular). Devuelve `total_monthly_estimate` (sum avg_amount × monthly_factor), `recurring_count`, `irregular_count`, `upcoming_count` (próximos 7 días), `overdue_count` (>7 días en pasado). Predice `next_expected_date` = last_date + avg_interval. Status: overdue/due/upcoming/on_track/irregular. Helpers: `_classify_frequency`, `_parse_iso_date`. **Frontend**: nueva página `/operacion` (Operations.jsx) con 4 StatCards (Estimado mensual fijo, Atrasados, Próximos 7d, Flujos recurrentes) + filtros por estado + tabla con Proveedor/Categoría/Frecuencia/Promedio/Último/Próximo/Estado/historial; **widget en Dashboard** (link al módulo) con borde rojo si hay overdue, ámbar si hay upcoming; link "Gastos Fijos" en sidebar entre "Ingresos y Egresos" y "Clientes". Testing: 13/13 backend pytest + Playwright 100% pass.
